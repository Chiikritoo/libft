...
static t_toml_value	*get_nested_value(t_toml_value *value, const char **path_parts, int part_count)
{
...
static char	**validate_and_parse_path(const char *path, int *part_count)
{
	...
	*part_count = 0;
	...
	if (*part_count < 2)
	{
		LOG(LOG_WARNING, "Invalid path format (need at least section.key): %s", path);
		ft_free_double_ptr((void ***)&parts);
		return (NULL);
	}
...
static t_toml_kv	*find_section_and_key(t_toml_doc *doc, char **parts)
{
	...
	if (!kv)
	{
		LOG(LOG_WARNING, "Key '%s' not found in section '%s'", key_name, section_name);
		return (NULL);
	}
...
static t_toml_value	*resolve_nested_path(t_toml_kv *kv, char **parts, int part_count)
{
	...
	nested_value = get_nested_value(&kv->value, (const char **)&parts[2], part_count - 2);
	return (nested_value);
}
