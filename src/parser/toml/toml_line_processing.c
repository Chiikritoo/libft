...
/*   Updated: 2025/07/30 01:18:48 by anchikri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_toml_section	*process_section_line(t_toml_doc *doc, const char *line, int line_num)
{
	...
	if (!section_name)
	{
		LOG(LOG_WARNING, "Invalid section format at line %d: %s", line_num, line);
		return (NULL);
	}
	...
	if (!section)
	{
		LOG(LOG_ERROR, "Failed to create section '%s' at line %d", section_name, line_num);
		free(section_name);
		return (NULL);
	}
...
bool	process_kv_line(t_toml_section *current_section, const char *line, int line_num)
{
	...
	if (!current_section)
	{
		LOG(LOG_WARNING, "Key-value pair outside section at line %d: %s", line_num, line);
		return (false);
	}
	...
	if (!kv)
	{
		LOG(LOG_WARNING, "Failed to parse key-value pair at line %d: %s", line_num, line);
		return (false);
	}

	current_section->kv = ft_realloc(current_section->kv,
		current_section->count * sizeof(t_toml_kv *),
		(current_section->count + 1) * sizeof(t_toml_kv *));
...
void	process_all_lines(t_toml_doc *doc, char **lines)
{
	...
	current_section = doc->sections[0]; // global section
	i = 0;
	while (lines[i])
	{
		current_section = process_single_line(doc, current_section, lines[i], i + 1);
		i++;
	}
} 
