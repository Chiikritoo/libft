...
/*   Updated: 2025/07/30 01:03:03 by anchikri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	is_toml_date(const char *str)
{
	...
	while (str[i])
	{
		if (str[i] == '-')
			...
			t_count++;
		else if (!ft_isdigit(str[i]) && str[i] != '.' && str[i] != 'Z' && str[i] != '+' && str[i] != '-')
			return (false);
		i++;
	}
	return (dash_count >= 2 && (t_count == 0 || (t_count == 1 && colon_count >= 2)));
}
...
bool	is_valid_toml_key(const char *key)
{
	...
	while (key[i])
	{
		...
		{
			LOG(LOG_WARNING, "Invalid character in key '%s': '%c'", key, key[i]);
			return (false);
		}
		i++;
	}
...
char	*remove_inline_comment(const char *str)
{
	...
	char *trimmed = ft_strtrim(result, " \t");
	free(result);
	return (trimmed);
}
...
char	*parse_toml_string(const char *str)
{
	...
	if (is_quoted)
	{
		...
		// TODO: Implement proper escape sequence handling
	}
	else
	{
		result = ft_strdup(str);
